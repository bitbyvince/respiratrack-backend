...
/**
 * Base helper — pipes a PDFDocument into a Buffer.
 * @param {function} buildFn - receives (doc) and draws content
...
/**
 * Generates a patient profile PDF.
 *
...
const generatePatientPdf = async (patient) => {
  return buildPdfBuffer((doc) => {
    ...
    drawRow(doc, 'Date of Diagnosis:',    patient.date_of_diagnosis?.toDateString());
    ...
    drawRow(doc, 'Compliance %:',         `${patient.compliance?.compliance_percentage}%`);
    ...
    drawRow(doc, 'Last Computed:',   patient.risk_score?.last_computed?.toDateString());
...
/**
 * Generates a barangay summary report PDF.
 *
 * @param {object} barangay      - Barangay document
 * @param {object[]} patients    - Array of patient documents for this barangay
 * @returns {Promise<Buffer>}
 */
const generateBarangayReportPdf = async (barangay, patients = []) => {
  return buildPdfBuffer((doc) => {
    ...
    patients.forEach((p, i) => {
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#333333')
        .text(`${i + 1}. ${p.full_name} (${p.tb_case_number})`, { continued: true })
        .font('Helvetica')
        .fillColor('#777777')
        .text(`  — ${p.compliance?.risk_level} | ${p.compliance?.compliance_percentage}%`);
    });
