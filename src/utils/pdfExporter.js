...
export const generateInventoryPDF = async (data) =>
  buildPdfBuffer((doc) => {
    ...
    (data.grouped_by_barangay || []).forEach((brgy) => {
      drawSectionHeader(doc, `Barangay: ${brgy.barangay_name || brgy.barangay_id}`);
...
export const generatePatientListPdf = async (patients = []) => {
  return buildPdfBuffer((doc) => {
    drawPageHeader(doc, "Patient List Report");
    ...
    patients.forEach((p, i) => {
      ...
      doc
        ...
        .fillColor("#777777")
        .text(
          `  — ${p.barangay_name || p.barangay_id} | ${p.compliance?.risk_level ?? "—"} | ${p.compliance?.compliance_percentage ?? 0}% compliant`,
        );
    });
