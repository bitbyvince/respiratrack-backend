...
const BarangaySchema = new Schema(
  {
    ...
    // Province code — not currently used by TB case number generation
    // (utils/caseNumberGenerator.js uses a fixed prefix instead). Kept
    // optional in case per-barangay case numbering is wired up later.
    province_code: {
      type: String,
      trim: true,
    ...
    // Municipality code — same status as province_code above.
    municipality_code: {
      type: String,
      trim: true,
      match: [
