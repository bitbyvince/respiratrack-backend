...
exports.createAlertSchema = Joi.object({
  ...
  alert_type: Joi.string().valid(...VALID_ALERT_TYPES).required().messages({
    'any.only': `alert_type must be one of: ${VALID_ALERT_TYPES.join(', ')}`,
  }),
  escalation_level: Joi.number().integer().min(0).max(3).optional(),
  message: Joi.string().min(5).required(),
  severity: Joi.string().valid(...VALID_SEVERITIES).required().messages({
    'any.only': `severity must be one of: ${VALID_SEVERITIES.join(', ')}`,
  }),
  target_roles: Joi.array().items(Joi.string().valid(...VALID_ROLES)).min(1).required(),
});

exports.resolveAlertSchema = Joi.object({
