...
exports.getBarangayAlerts = async (req, res) => {
  try {
    const { barangayId } = req.params;
    const { status, severity, alert_type } = req.query;
    const alerts = await service.getBarangayAlerts(barangayId, {
      status,
      severity,
      alert_type,
    });
    return res
      .status(200)
      .json(success("Barangay alerts retrieved.", { alerts }));
  } catch (err) {
    return res.status(400).json(error(err.message));
  }
};
...
exports.getPatientAlerts = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(success("Patient alerts retrieved.", { alerts }));
  } catch (err) {
    return res.status(400).json(error(err.message));
  }
};
