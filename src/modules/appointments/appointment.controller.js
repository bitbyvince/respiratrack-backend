...
export const getBarangayAppointments = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 'Barangay appointments retrieved.', { appointments });
  } catch (err) {
    return sendError(res, err);
  }
};
...
export const completeAppointment = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 'Appointment marked as completed.', { appointment });
  } catch (err) {
    return sendError(res, err);
  }
};
...
export const getMyAppointments = async (req, res) => {
  try {
    ...
    const appointments = await service.getPatientAppointments(req.user.patient_id, { status, upcoming, page, limit });
