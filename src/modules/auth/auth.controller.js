...
export async function changePin(req, res) {
  try {
    ...
    return sendSuccess(res, "PIN updated successfully.", null, 200);
  } catch (err) {
    return sendError(res, err);
  }
}
...
export async function requestOtp(req, res) {
  try {
    ...
    return sendSuccess(res, "OTP sent to the provided phone number.", null, 200);
  } catch (err) {
    return sendError(res, err);
  }
}
...
export async function getMe(req, res) {
  try {
    ...
    return sendSuccess(res, "User retrieved.", user, 200);
  } catch (err) {
    return sendError(res, err);
  }
}
...
export default {
  staffLogin, patientLogin, setPatientPin, refreshToken,
  logout, changePassword, changePin, requestOtp, verifyOtp, getMe,
};