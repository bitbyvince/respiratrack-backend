...
import { computeRiskScore, getRiskLabel } from "../utils/riskScoring.js";
  ...
  try {
    ...
    for (const barangay of barangays) {
      ...
      if (patients.length === 0) {
        logger.info(
          `[complianceSnapshot.job] No active patients in ${barangay.name}, skipping.`,
        );
        continue;
      }
    ...
    logger.info(
      "[complianceSnapshot.job] Daily compliance snapshot completed.",
    );
  } catch (err) {
...
export const scheduleComplianceSnapshot = () => {
  cron.schedule("59 23 * * *", runComplianceSnapshot, {
    timezone: "Asia/Manila",
  });
  logger.info(
    "[complianceSnapshot.job] Scheduled — daily at 23:59 Asia/Manila",
  );
};

export { runComplianceSnapshot };
